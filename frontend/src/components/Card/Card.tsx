...
const STYLE_CONFIG: Record<string, { folder: string; back: string }> = {
...
function ImageFace({ rank, suit, selected, cardStyle }: { rank: number; suit: SuitInfo; selected?: boolean; cardStyle: string }): React.JSX.Element {
  const rankCode = rankLabel(rank);
...
function ImageBack({ cardStyle }: { cardStyle: string }): React.JSX.Element {
...
function StandardFace({ label, suit, colorClass, selected }: FaceProps): React.JSX.Element {
  return (
...
function ClassicFace({ label, suit, colorClass, selected }: FaceProps): React.JSX.Element {
  return (
...
function MinimalFace({ label, suit, colorClass, selected }: FaceProps): React.JSX.Element {
  return (
...
function ClassicStyleFace({ label, suit, colorClass, selected, rank }: FaceProps): React.JSX.Element {
...
function ModernStyleFace({ label, suit, colorClass, selected, rank }: FaceProps): React.JSX.Element {
  const isFaceCard = rank >= 11;
  return (
...
function FantasyStyleFace({ label, suit, colorClass, selected, rank }: FaceProps): React.JSX.Element {
  const isFaceCard = rank >= 11;
  const isAce      = rank === 1;
  return (
...
export function Card({ card, faceUp, selected, onClick, design: designProp }: CardProps): React.JSX.Element {
  ...
  const faceProps: FaceProps = { label, suit, colorClass, selected, rank };
...
export function CardSlot({ onClick, children }: { onClick?: () => void; children?: React.ReactNode }): React.JSX.Element {
  return (
